...
from pom.stochastic03.InitData.inizialize_data03 import experiments
import pandas as pd
...
class InputFlow3:
    ...
    def execute_initial_correlation_aproximate(self):
        print ("\nexecute_initial_correlation_aproximate ")
        period = self.experiment["period"]
        self.initial_correlation = pd.DataFrame()
        self.initial_correlation['time'] = [0.0] * round(self.numberExamples/period/2.0)
        self.initial_correlation['correlation'] = [0.0] * round(self.numberExamples/period/2.0)
...
#=======================================================================================================================
    ...
    def gamma_optimum_spectrum_show(self):
        ...
        size = len(self.fourier_coefficient_for_gamma_d)
        fourier_coefficient = [0] * size
        for n in range(size):
            if(n==0):
                fourier_coefficient[n] = self.fourier_coefficient_for_gamma_d[n]/2
            else:
                fourier_coefficient[n] = self.fourier_coefficient_for_gamma_d[n]

        gamma_optimum_spectrum_s = [range(size), fourier_coefficient]
        show.common_bar(self.experiment, path, gamma_optimum_spectrum_s, "gamma_optimum_spectrum_s_bar_", "gamma_optimum_s_bar")
...
#=========================================================
    ...
    def paremeter_model_save(self):
        ...
        file.write("\n")
        file.write("The optimization criterium value         : %10.7f" % (self.criterium))
        file.write("\n")
        file.write("The optimization correlation tau value   : %10.7f" % (self.correlation_tau_optimum))
        file.write("\n")
        file.write("The optimization standard deviation value: %10.7f" % (self.standard_deviation_optimum))
        file.write("\n")
        file.write("The optimization type id value           : %10.7f" % (self.type_id_optimum))
        file.write("\n")
        file.write("Fourier coefficients for gamma d   :")
        file.write("\n")
        for n in range(len(self.fourier_coefficient_for_gamma_d)):
            file.write("  n: %2d  " % n)
            file.write("%10.5f" % self.fourier_coefficient_for_gamma_d[n])
        file.write("\n")
